...
//            getActivity().getSupportFragmentManager().beginTransaction().addToBackStack(null).replace(R.id.contentView, regist_fragment).commit();
           ...
           LoginSharedPreferences preferences = new LoginSharedPreferences(getActivity());
           preferences.setName(Name.getText().toString().trim());
           preferences.setPassword(Password.getText().toString().trim());
